...
let graph_of_map map =
  ...
  for i = 1 to height - 2 do
    for j = 1 to width - 2 do
      if map.(i).(j) <> F then
        ...
        | 2 -> failwith "A degree-2-node remains"
  ...
  let start = ref (-1)
  and stop = ref (-1)
  and nodes = Hashtbl.length graph in
  let g = Array.make nodes []
  and pos_to_int = Hashtbl.create 10
  and int_to_pos = Hashtbl.create 10 in
  let cnt = ref 0 in
  Hashtbl.iter
    (fun ((lin, _) as pos) _ ->
      Hashtbl.add pos_to_int pos !cnt;
      Hashtbl.add int_to_pos !cnt pos;
      if lin = 0 then start := !cnt;
      if lin = height - 1 then stop := !cnt;
      incr cnt)
    graph;
  assert (!start >= 0);
  assert (!stop >= 0);
  for i = 0 to !cnt - 1 do
    g.(i) <-
      Hashtbl.fold
        (fun pos len lst ->
          (Hashtbl.find pos_to_int pos, len) :: lst)
        (Hashtbl.find int_to_pos i |> Hashtbl.find graph)
        []
  done;
  (g, !start, !stop)
...
(*       Hashtbl.iter *)
(*         (fun nxt len -> *)
(*           if not (Hashtbl.mem seen nxt) then *)
(*             visit nxt (steps + len)) *)
...
let dfs2 graph start stop =
  let nb = Array.length graph in
  let seen = Array.make nb false and max_steps = ref 0 in
  let rec visit pos steps =
    (* Printf.printf "%d %d %d\n" lin col steps; *)
    if pos = stop then (
      if steps > !max_steps then max_steps := steps)
    else (
      (* assert (seen.(pos) = false); *)
      seen.(pos) <- true;
      List.iter
        (fun (nxt, len) ->
          if not seen.(nxt) then visit nxt (steps + len))
        graph.(pos);
      seen.(pos) <- false)
  in
  visit start 0;
...
let _ =
  ...
  let graph, start, stop = graph_of_map map in
  Printf.printf "Part two: %d\n" (dfs2 graph start stop)
